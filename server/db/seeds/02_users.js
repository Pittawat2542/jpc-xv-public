...
for (let i = 0; i < 50; i++) {
  usersData.push({
    id: i + 1,
    th_first_name: "รักชาติ",
    th_last_name: "เรียนดี",
    en_first_name: users[Math.floor(Math.random() * 50)].name.first,
    en_last_name: users[Math.floor(Math.random() * 50)].name.last,
    nick_name: "ชาติ",
    gender:
      users[Math.floor(Math.random() * 50)].gender === "male" ? "ชาย" : "หญิง",
    birth_date: new Date(users[Math.floor(Math.random() * 50)].dob.date),
    religion: "พุทธศาสนา",
    ...
    medicine_allergy: "",
    blood_group: "A",
    phone_number: users[Math.floor(Math.random() * 50)].phone,
    email: users[Math.floor(Math.random() * 50)].email,
    parent_name: "สมหมาย เรียนดี",
    parent_phone_number: users[Math.floor(Math.random() * 50)].phone,
    parent_relation: "บิดา",
    school: "โรงเรียนใฝ่รู้",
    education_level: "ม.6",
    education_program: "วิทย์ - คณิต",
    gpa: 4.0,
    grade_report_file_name: "document-1560136080980.jpg",
    profile_picture_file_name: "picture-1560179322516.jpg",
    answer_id: i + 1,
...
exports.seed = function(knex, Promise) {
  return knex("jpc_xv_users")
    .del()
    .then(function() {
      return knex("jpc_xv_users").insert(usersData);
    });
};
